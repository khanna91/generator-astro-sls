...
describe('Utils - APIError', () => {
  ...
  it('should generate unspecified error', () => {
    const sut = APIError.withCode();
  ...
  it('should generate unknown error with 500 status', () => {
    const sut = APIError.withCode('UNKNOWN', 500, { customAttributes: 'test' });
    ...
    expect(sut.message).toEqual(expect.any(String));
    expect(sut.route).toEqual(expect.any(String));
    expect(sut.status).toEqual(httpStatus.INTERNAL_SERVER_ERROR);
    expect(sut.isPublic).toEqual(expect.any(Boolean));
  });
});
